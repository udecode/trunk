export type TreeEditorError<T extends TreeErrorType = TreeErrorType> = {
  /**
   * The unique key of the operation and the specific error. The key format:
   * `<methodName>.<case>`
   */
  key: string;
  ...
  message: string;
  /**
   * Additional data related to the operation that caused the error for custom
   * handling.
   */
  data?: unknown;
  ...
  error?: TreeError<T>;
  /**
   * Experimental recovery value instead of throwing an error. Used as returned
   * value only if `editor.strict = false`.
   */
  recovery?: unknown;
};

export type TreeError<T extends TreeErrorType = TreeErrorType> = {
  /** Error description. */
  message: string;
  /** Error type. */
  type: T;
...
export type TreeErrorType = keyof typeof TreeErrors;
